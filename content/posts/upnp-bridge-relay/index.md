...
两个 ipk 包：`upnp-bridge-relay` 是核心，`luci-app-upnp-bridge-relay` 是网页管理界面。在 OpenWrt 上装就行：

```sh
opkg install upnp-bridge-relay luci-app-upnp-bridge-relay
```

OpenWrt 25.12 以后用 `apk`：

```sh
apk add upnp-bridge-relay luci-app-upnp-bridge-relay
```

装完之后刷新一下 LuCI 页面，就能在「服务」菜单下面看到 UPnP Bridge Relay 了。依赖会自动拉上：`miniupnpc`、`nftables`、`uci`、`ubus`——都是 OpenWrt 上本来就该有的东西，不会给你塞一堆莫名其妙的新依赖。这点我很满意，有的工具装一个带八个，跟搬家似的，烦死了。
...
**端口白名单要认真设。** 默认是放行所有端口——不是因为我懒，是给你最大的灵活度。但你最好还是设个范围，1024-65535 就挺合理。别把 22、80、443 这些管理端口映射出去，不然你的路由器就变成全网的公共厕所了，谁都来逛一圈。
