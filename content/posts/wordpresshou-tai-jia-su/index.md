    ...
    add_action('init', function () {
         ob_start(function ($buffer) {
             ...
             if (is_plugin_active('woocommerce/woocommerce.php')) {
                 ...
                 $buffer = preg_replace('~'.home_url('/').'wp-content/plugins/woocommerce/(._)._(css|js|woff|woff2|jpg|png|svg|webp)_ ~', sprintf('//cdn.jsdelivr.net/gh/woocommerce/woocommerce@%s/$1.$2', $plugin_data['Version']), $buffer);_
             _}_
    _if (is_plugin_active('autoptimize/autoptimize.php')) {_
    _$plugin_data = get_plugin_data(WP_PLUGIN_DIR . '/autoptimize/autoptimize.php');_
    _$buffer = preg_replace('~'.home_url('/').'wp-content/plugins/autoptimize/(._)._(css|js|woff|woff2|jpg|png|svg|webp)_ ~', sprintf('//cdn.jsdelivr.net/gh/futtta/autoptimize@%s/$1.$2', $plugin_data['Version']), $buffer);
             }
             return $buffer;
         });
     });
    ...
    add_action('init', function () {
         ob_start(function ($buffer) {
             ...
             if (WOOCOMMERCE_VERSION) {
                 $buffer = preg_replace('~'.home_url('/').'wp-content/plugins/woocommerce/(._).(css|js|woff|woff2|jpg|png|svg|webp)~', sprintf('//cdn.jsdelivr.net/wp/plugins/woocommerce/tags/%s/$1.$2', WOOCOMMERCE_VERSION), $buffer);_
    _}_
    _if (AUTOPTIMIZE_PLUGIN_VERSION) {_
    _$buffer = preg_replace('~'.home_url('/').'wp-content/plugins/autoptimize/(._).(css|js|woff|woff2|jpg|png|svg|webp)~', sprintf('//cdn.jsdelivr.net/wp/plugins/autoptimize/tags/%s/$1.$2', AUTOPTIMIZE_PLUGIN_VERSION), $buffer);
             }
             return $buffer;
         });
     });
